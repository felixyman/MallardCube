...
`ProductCategory` to Rows and Filters, the filter dropdown works, and changing
...
- Removed `PARENT_UNIQUE_NAME` from `product_category_dim_props_all()` —
...
- `src/backend.rs` with in-memory SQLite (`rusqlite`): table `fact_table`
  with `product_category` + `sales`, demo rows for Category A/B/C/D.
- `Execute` now uses `Backend` for totals and grouped values instead of
  hard-coded `1250000.5`.

### Known limitation
- `parse_category_filter` matches `[ProductCategory]` anywhere in the MDX
...
xmla_proxy/src/
  ...
  tables.rs            — DBSCHEMA_TABLES (FactTable SYSTEM TABLE / MEASURE_GROUP +
                         ProductCategory TABLE / CUBE_DIMENSION)
  dimensions.rs        — MDSCHEMA_DIMENSIONS (2 dims: Measures TYPE=2 hidden,
                         ProductCategory TYPE=3 visible) — includes DIMENSION_GUID,
                         DIMENSION_MASTER_UNIQUE_NAME, CUBE_SOURCE
  hierarchies.rs       — MDSCHEMA_HIERARCHIES (1 hierarchy: [ProductCategory].[ProductCategory])
  levels.rs            — MDSCHEMA_LEVELS (2 levels: (All) hidden + ProductCategory visible)
  measures.rs          — MDSCHEMA_MEASURES (1 measure: Total Sales, with DAX EXPRESSION)
  measure_groups.rs    — MDSCHEMA_MEASUREGROUPS (FactTable)
  measuregroup_dimensions.rs — MDSCHEMA_MEASUREGROUP_DIMENSIONS
                         (FactTable↔[Measures], FactTable↔[ProductCategory])
  ...
  backend.rs           — SQLite demo backend (rusqlite): fact_table schema,
...
- PivotTable Fields renders correctly: `Σ FactTable` + `Total Sales (SEK)`,
  and `ProductCategory` table + `ProductCategory` hierarchy.
...
2. **DuckDB demo backend.** Execute the semantic query model against a single narrow
   schema (`product_category`, `sales`) so filters and grouped totals return real data.

...
`ProductCategory`, `Region`, and `Total Sales`. DuckDB is the
...
- `build_drilldown_member()` handles three collapse branches:
  1. Region collapse (excluded ProductCategory)
  2. ProductCategory collapse (excluded ProductCategory)
  3. ProductCategory collapse (excluded Region) — symmetric reverse case
...
- Cube name: `Model`
- Measure: `Total Sales` (caption `Total Sales (SEK)`)

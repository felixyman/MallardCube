...
**Current status:** Excel renders the PivotTable Fields panel correctly with `Σ FactTable` (containing `Total Sales (SEK)`) and `ProductCategory` (containing the `ProductCategory` hierarchy). Drag/drop into the pivot has not been verified end-to-end yet.
...
xmla_proxy/src/
  ...
  members.rs           — MDSCHEMA_MEMBERS (2 rows for ProductCategory)
...
3. **Real DuckDB integration.** Load a demo Parquet/CSV, issue SQL for the
   ProductCategory drilldown, encode results as a cellset. First truly live pivot.

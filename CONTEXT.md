...
probe, query, collapse/expand) on `ProductCategory`, `Region`, and
`Total Sales`. 81 unit tests. Full pipeline: `MDX -> ParsedMdx ->
...
### Typed semantic IR
- Added `Dimension` enum (`ProductCategory`, `Region`) and `Measure` enum
  (`TotalSales`).
...
- `default_model()`: static model for `fact_table` dataset.
...
- Cube dimension ordinal order: `ALL_DIMS = ["Measures", "ProductCategory", "Region"]`

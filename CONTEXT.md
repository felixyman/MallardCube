...
**Current status:** Excel can place `ProductCategory` and `Region` on Rows,
both together or independently, filter by either dimension, and add
`Total Sales` to Values. All three cube dimensions (Measures,
ProductCategory, Region) work correctly through the full metadata→probe→query
...
- `build_drilldown_member()` in `execute_builders.rs` handles two collapse
  forms:
  - Collapse Region under ProductCategory: excluded category stays as
    `(Category X, Region.All)` with category total.
  - Collapse ProductCategory under Region: excluded category collapses to
    `(ProductCategory.All, Region)` per region with pair value.
...
- This was the root cause of "Total Sales won't go into Values."

### Complete SlicerAxis (all off-axis dimensions)
- `full_slicer_axis()` now includes EVERY cube dimension not on the visible
  axis, in stable metadata-ordinal order (Measures, ProductCategory, Region).
...
- Backend: added `region` column to `fact_table`, 8 demo rows covering
  North/South × Category A-D.
...
- Combined queries: `grouped_by_product_category(region_filter)`,
...
- `product_category_dim_props_all()` no longer emits `PARENT_UNIQUE_NAME`.
...
xmla_proxy/src/
  ...
  tables.rs            — DBSCHEMA_TABLES (FactTable, ProductCategory, Region)
  dimensions.rs        — MDSCHEMA_DIMENSIONS (Measures hidden, ProductCategory, Region)
  hierarchies.rs       — MDSCHEMA_HIERARCHIES ([Measures], [ProductCategory], [Region])
  levels.rs            — MDSCHEMA_LEVELS (MeasuresLevel, Prod.All/Prod, Region.All/Region)
  measures.rs          — MDSCHEMA_MEASURES (Total Sales)
  measure_groups.rs    — MDSCHEMA_MEASUREGROUPS (FactTable)
  measuregroup_dimensions.rs — MDSCHEMA_MEASUREGROUP_DIMENSIONS
  members.rs           — MDSCHEMA_MEMBERS (ProductCategory + Region members)
  ...
  backend.rs           — SQLite backend (rusqlite): fact_table with
                         product_category, region, sales; grouped/filtered queries
...
- PivotTable Fields renders: `Σ FactTable`, `Total Sales (SEK)`,
...
- **Single-dimension drilldown**: `DrilldownLevel` for ProductCategory or Region.
- **Two-dimension CrossJoin**: `ProductCategory` and `Region` both on Rows
...
- **Cross-dimension filtering**:
  - ProductCategory on Rows + Region in Filter
  - Region on Rows + ProductCategory in Filter
...
- `MDSCHEMA_MEMBERS` filter logic is dimension-agnostic (PARENT/ANCESTORS
  use actual parent names, not hardcoded ProductCategory All).
...
18. **`DrilldownMember(CrossJoin(...), excluded_set, hierarchy)` is the
    ...
    forms observed:
    - Collapse Region under ProductCategory → `(Kat, Region.All)` with cat total
    - Collapse ProductCategory under Region → `(Prod.All, Region)` per region

## Hard-coded constants
- Catalog name: `KTH_KEX_MALLOY_CUBE`
- Cube name: `Model`
- Measure name: `Total Sales` (caption `Total Sales (SEK)`)
- Measure group: `FactTable`
- Dimensions: `ProductCategory`, `Region`
- Session ID: `RUST-SESSION-456` (in response.rs)
- Cube dimension ordinal order: `ALL_DIMS = ["Measures", "ProductCategory", "Region"]` (in execute_builders.rs)

-- Cumulative YTD for: Antal utförda remisser (ack vecka) CY
-- Original DAX: CALCULATE( [Antal utförda remisser], FILTER( ALLSELECTED('dw_fys Kalender_Undersökningsslut'[Vecka]), ISONORAFTER('dw_fys Kalender_Undersökningsslut'[Vecka],    MAX('dw_fys Kalender_Undersökningsslut'[Vecka]), DESC) ),'dw_fys Kalender_Undersökningsslut'[ÅR] = YEAR(TODAY()) )

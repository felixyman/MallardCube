...
1. `Hierarchy` attribute: value is the hierarchy unique name (e.g., `[ProductCategory].[ProductCategory]`)
...
with both ASCII and UTF-8 (non-ASCII characters like `ü` in a caption are fine).

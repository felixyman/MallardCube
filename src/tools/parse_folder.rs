...
fn parse_partitions(dir: &Path) -> Vec<PartitionInfo> {
    ...
    for entry in entries.flatten() {
            ...
            && let Ok(v) = serde_json::from_str::<serde_json::Value>(&text)
        {
            ...
            let data_source_name = v["source"]["dataSource"].as_str().map(|s| s.to_string());
...
fn parse_table_schema_annotation(
    annotations: &serde_json::Value,
) -> (Option<String>, Option<String>) {
    if let Some(arr) = annotations.as_array() {
        for ann in arr {
            if ann["name"].as_str() == Some("TabularEditor_TableSchema")
                && let Some(val_str) = ann["value"].as_str()
                && let Ok(val) = serde_json::from_str::<serde_json::Value>(val_str)
            {
...
fn parse_roles(dir: &str) -> Vec<RoleInfo> {
    ...
    for entry in entries.flatten() {
            ...
            && let Ok(v) = serde_json::from_str::<serde_json::Value>(&text)
        {
            ...
            let model_permission = v["modelPermission"].as_str().unwrap_or("read").to_string();
...
mod tests {
    ...
    fn test_ado_connection_string_parser() {
        ...
        assert_eq!(ado_server(&map), "DESKTOP-PONL6H6\\MSSQLSERVER01");

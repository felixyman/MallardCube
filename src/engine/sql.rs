...
mod tests {
    ...
    fn sql_total_no_filters() {
        ...
        assert_eq!(sql, "SELECT SUM(sales) FROM fact_table");
    }
    ...
    fn sql_total_with_filter() {
        ...
        let sql = sql_for_query_plan(&default_model(), &plan);
        assert!(sql.contains("SELECT SUM(sales) FROM fact_table"));
    ...
    fn sql_group_by_one_dim() {
        let plan = QueryPlan::GroupBy {
            measure: Measure::TotalSales,
            group_by: vec![Dimension::ProductCategory],
            filters: vec![],
        };
        let sql = sql_for_query_plan(&default_model(), &plan);
        assert_eq!(
            sql,
            "SELECT product_category, SUM(sales) FROM fact_table GROUP BY 1 ORDER BY 1"
        );
    }
    ...
    fn sql_group_by_two_dims() {
        let plan = QueryPlan::GroupBy {
            measure: Measure::TotalSales,
            group_by: vec![Dimension::ProductCategory, Dimension::Region],
            filters: vec![],
        };
        let sql = sql_for_query_plan(&default_model(), &plan);
        assert_eq!(
            sql,
            "SELECT product_category, region, SUM(sales) FROM fact_table GROUP BY 1, 2 ORDER BY 1, 2"
        );
    }
    ...
    #[test]
    fn sql_count() {
        let plan = QueryPlan::Count { dimension: Dimension::ProductCategory };
        let sql = sql_for_query_plan(&default_model(), &plan);
        assert_eq!(sql, "SELECT COUNT(DISTINCT product_category) FROM fact_table");
    }
    ...
    fn sql_total_multi_filter_both_dims() {
        let plan = QueryPlan::Total {
            ...
            filters: vec![
                ...
                TypedDimensionFilter {
                    dimension: Dimension::ProductCategory,
        ...
        let sql = sql_for_query_plan(&default_model(), &plan);
        assert!(sql.contains("WHERE region IN ('North')"));
        assert!(sql.contains("AND product_category IN ('Category A', 'Category B')"));
    }
}

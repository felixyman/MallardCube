...
mod tests {
    ...
    fn sql_group_by_one_dim() {
        ...
        assert_eq!(
            sql,
            "SELECT product_category, SUM(sales) FROM fact_table f GROUP BY 1 ORDER BY 1"
        );
    }
    ...
    fn sql_group_by_two_dims() {
        ...
        assert_eq!(
            sql,
            "SELECT product_category, region, SUM(sales) FROM fact_table f GROUP BY 1, 2 ORDER BY 1, 2"
        );
    }

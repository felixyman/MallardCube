    ...
    for fact in &model.fact_tables {
        ...
        for dim in &model.dimensions {
            if dim.semantic_name != dim.physical_field {
                lines.push(format!(
                    "  dimension: {} is {}",
                    dim.semantic_name, dim.physical_field,
                ));
            }
        }
        ...
        for meas in &model.measures {
            if meas.fact_table_idx == blocks.len() {
                lines.push(format!(
                    "  measure: {} is {}",
                    meas.semantic_name, meas.physical_expr,
                ));
            }
        }
...
pub fn malloy_query(model: &SemanticModel, plan: &QueryPlan) -> String {
    match plan {
        ...
        QueryPlan::GroupBy {
            ...
            let dim_names: Vec<&str> = group_by
                .iter()
        ...
        QueryPlan::Count { .. } => String::from("-- Count probe: no Malloy equivalent\n"),

        QueryPlan::Empty => String::from("-- Empty probe: no Malloy equivalent\n"),
    }
}
...
fn where_clause(model: &SemanticModel, filters: &[TypedDimensionFilter]) -> Option<String> {
    let dim_parts: Vec<String> = filters
        .iter()
        .filter(|f| !f.members.is_empty())
        .filter_map(|f| {
            model
                .dim_def_opt(&f.dimension)
                .map(|d| (d.semantic_name.as_str(), &f.members))
        })
        .map(|(dim_name, members)| {
            let member_conds: Vec<String> = members
                .iter()
...
mod tests {
    ...
    fn model_emits_measures_and_source() {
        ...
        assert!(out.contains("source: fact_table is duckdb.table('fact_table')"));
        ...
        assert!(query.contains("group_by: product_category"));
    }
    ...
    fn query_group_by_one_dim() {
        ...
        let out = malloy_query(&default_model(), &plan);
        assert!(out.contains("group_by: product_category"));
        assert!(out.contains("aggregate: total_forsaljning"));
    }
    ...
    fn query_group_by_two_dims() {
        ...
        let out = malloy_query(&default_model(), &plan);
        assert!(out.contains("group_by: product_category, region"));
    }
    ...
    fn query_two_dim_filters_use_and() {
        ...
        assert!(out.contains("product_category = 'Category A'"));
        assert!(
            !out.contains(" | "),
            "different dims must not use OR: {out}"
        );
    }
    ...
    fn query_same_dim_multi_member_uses_or() {
        ...
        assert!(out.contains("(product_category = 'Category A' or product_category = 'Category B')"));
    }
    ...
    fn query_mixed_filters_handles_both() {
        ...
        let out = malloy_query(&default_model(), &plan);
        // Region = single member, ProductCategory = multi-member
        assert!(out.contains("region = 'North'"));
        assert!(out.contains("(product_category = 'Category A' or product_category = 'Category B')"));
    ...
    use crate::engine::model::{
        Dialect as ModelDialect, DimensionDef, FactTable, MeasureDef, SemanticModel,
    };
    ...
    fn malloy_model_emits_multiple_sources() {
        ...
        assert!(
            ...
            "should have sales source: {out}"
        );
        assert!(
            out.contains("source: inv_data"),
            "should have inventory source: {out}"
        );
    }
    ...
    fn malloy_model_puts_measures_in_correct_source() {
        ...
        assert!(
            rev_pos > sales_pos && rev_pos < inv_pos,
            "Revenue should be under sales_data block"
        );
        assert!(stock_pos > inv_pos, "Stock should be under inv_data block");
    }

    #[test]
    fn query_uses_measure_source_for_total() {
        let model = two_fact_model();
        let out = malloy_query(
            &model,
            &QueryPlan::Total {
        ...
        assert!(
            out.contains("run: sales_data ->"),
            "Revenue Total should use sales_data: {out}"
        );
        let out = malloy_query(
            &model,
            &QueryPlan::Total {
        ...
        assert!(
            out.contains("run: inv_data ->"),
            "Stock Total should use inv_data: {out}"
        );
    }
    ...
    fn query_uses_measure_source_for_group_by() {
        ...
        assert!(
            out.contains("run: inv_data ->"),
            "Stock GroupBy should use inv_data: {out}"
        );
    }
}

...
fn dim_name(input: &str) -> IResult<&str, DimKey> {
    alt((
        value(DimKey::ProductCategory, tag("ProductCategory")),
...
fn detect_axis_dimension(input: &str) -> DimKey {
    if input.contains("[Region]") {
        DimKey::Region
    } else if input.contains("[ProductCategory]") {
        DimKey::ProductCategory
    } else {
        DimKey::Measures
    }
}
...
fn detect_cchildren_target(input: &str) -> CChildrenTarget {
    ...
    if set.contains("[Measures]") && !set.contains("[ProductCategory]") && !set.contains("[Region]") {
        return CChildrenTarget::Measures;
    }

    if (set.contains("[ProductCategory]") || set.contains("[Region]"))
...
fn is_slicer_all_measure(input: &str) -> bool {
    input.contains("WHERE ([ProductCategory].[ProductCategory].[All],[Measures].[Total Sales])")
        || input.contains("WHERE ([Region].[Region].[All],[Measures].[Total Sales])")
}
...
mod tests {
    ...
    fn parse_member_all() {
        ...
        assert_eq!(m, MemberRef::All(DimKey::ProductCategory));
    ...
    fn parse_member_leaf() {
        ...
        assert_eq!(m, MemberRef::Leaf { dim: DimKey::ProductCategory, key: "Category A".into() });
        assert!(rest.is_empty());
    }
    ...
    fn parse_where_leaf() {
        ...
        let (rest, members) = where_clause(input).unwrap();
        assert_eq!(members.len(), 2);
        assert_eq!(members[0], MemberRef::Leaf { dim: DimKey::ProductCategory, key: "Category B".into() });
    }

    #[test]
    fn parse_subquery() {
        let input = "SELECT ({[ProductCategory].[ProductCategory].&[Category A],[ProductCategory].[ProductCategory].&[Category C]}) ON COLUMNS FROM [Model]";
        let (rest, m) = subquery_body("SELECT ({[ProductCategory].[ProductCategory].&[Category A],[ProductCategory].[ProductCategory].&[Category C]})").unwrap();
        assert_eq!(m.len(), 2);
    }

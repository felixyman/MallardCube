...
fn row_dim(query: &SemanticQuery) -> &str {
    query.axis_dimensions.first().map(|s| s.as_str()).unwrap_or("ProductCategory")
}
...
fn kat_filter(filters: &[DimensionFilter]) -> Vec<String> {
    filters.iter().find(|f| f.dimension == "ProductCategory")
...
fn fetch_grouped(row_dim: &str, filters: &[DimensionFilter]) -> Vec<(String, f64)> {
    ...
    match row_dim {
        ...
        _ => backend.grouped_by_product_category(&rf, &kf),
    }
}
...
const ALL_DIMS: &[&str] = &["Measures", "ProductCategory", "Region"];

/// Build a SlicerAxis that includes every cube dimension not on the visible axis,
/// in stable metadata-ordinal order (Measures, ProductCategory, Region).
...
fn build_drilldown(query: &SemanticQuery) -> String {
    ...
    let dim = dims.first().map(|s| s.as_str()).unwrap_or("ProductCategory");
...
fn build_drilldown_multi(query: &SemanticQuery) -> String {
    ...
    for (kat, region, value) in &all_data {
        ...
        let kat_member = leaf_member_for("ProductCategory", kat, &query.dim_props);
...
fn build_drilldown_member(query: &SemanticQuery) -> String {
    ...
    for (kat, region, value) in &all_data {
        ...
        if collapse_hier == "Region" && is_excluded {
            if !seen_kats.contains(kat.as_str()) {
                ...
                tuples.push(cellset::TupleConfig {
                    members: vec![
                        leaf_member_for("ProductCategory", kat, &query.dim_props),
        ...
        if collapse_hier == "ProductCategory" && is_excluded {
            if !seen_regions.contains(region.as_str()) {
                tuples.push(cellset::TupleConfig {
                    members: vec![
                        all_member_for("ProductCategory", &query.dim_props),

...
fn map_pair_values<'a>(dims: &[String], first: &'a str, second: &'a str) -> (&'a str, &'a str) {
    let dim0 = dims.first().map(|s| s.as_str()).unwrap_or("ProductCategory");
    if dim0 == "Region" {
        (second, first)
    } else {
        (first, second)
...
fn build_drilldown_member(query: &SemanticQuery, result: &QueryResult) -> String {
    ...
    let excluded_kats: std::collections::HashSet<&str> = query.excluded_members.iter()
        .filter(|e| e.dimension == "ProductCategory")
        .map(|e| e.key.as_str())
        .collect();
    ...
    for (first, second, value) in all_data {
        ...
        // ProductCategory collapse: excluded by ProductCategory member
        if collapse_hier == "ProductCategory" && is_kat_excluded {
            if !seen_regions.contains(region) {
                let kat_all = all_member_for("ProductCategory", &query.dim_props);
                let region_leaf = leaf_member_for("Region", region, &query.dim_props);
                tuples.push(ordered_pair(
                    dims,
                    "ProductCategory", kat_all,
                    "Region", region_leaf,
                ));
                cells.push(measurement_cell(ordinal, *value));
                ordinal += 1;
        ...
        // ProductCategory collapse: excluded by Region member
        if collapse_hier == "ProductCategory" && is_region_excluded {
            if !seen_regions.contains(region) {

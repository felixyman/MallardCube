...
fn row_dim(query: &SemanticQuery) -> &str {
    query.row_dimension.as_deref().unwrap_or("ProductCategory")
}
...
fn fetch_grouped(row_dim: &str, filters: &[DimensionFilter]) -> Vec<(String, f64)> {
    ...
    match row_dim {
        ...
        _ => backend.grouped_by_product_category(&region_filter(filters)),
    }
}
...
fn build_crossjoin_probe(query: &SemanticQuery) -> String {
    // CrossJoin(DrilldownLevel(ProductCategory.All), DrilldownLevel(Region.All))
    ...
    let dim = "ProductCategory";
    let data = Backend::get().grouped_by_product_category(&[]);
    let members = leaf_members_from(dim,
        &data.iter().map(|(n, _)| n.clone()).collect::<Vec<_>>(),
        &query.dim_props,
    );

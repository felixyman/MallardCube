...
mod tests {
    ...
    fn parse_mdx_filters_uses_slicer_not_subquery_when_slicer_has_product() {
        ...
        let kat = filters.iter().find(|f| f.dimension == "ProductCategory").unwrap();
    ...
    fn slicer_axis_has_region_all_for_kat_rows_query() {
        ...
        assert!(caps.iter().any(|c| c.contains("Total Sales")),
                "SlicerAxis missing Total Sales, caps: {:?}", caps);
    ...
    fn kat_filter_single_returns_only_filtered_category() {
        ...
        assert!(!xml.contains("Category A"), "Category A should be filtered out");
        assert!(!xml.contains("Category C"), "Category C should be filtered out");
    }
    ...
    fn nested_filters_parse_both_dimensions() {
        ...
        let kat = filters.iter().find(|f| f.dimension == "ProductCategory")
        ...
        assert!(!kat.contains(&"Category C".to_string()), "Category C should be filtered out");
    ...
    fn collapse_kategori_a_keeps_all_tuple() {
        ...
        assert!(xml.contains("Category A"), "Category A should remain visible as All");
    ...
    fn collapse_product_category_detected() {
        ...
        assert_eq!(q.kind, SemanticQueryKind::DrilldownMemberProbe);
        assert_eq!(q.drilldown_member_hierarchy.as_deref(), Some("ProductCategory"));
        assert_eq!(q.excluded_members.len(), 1);
        assert_eq!(q.excluded_members[0].dimension, "ProductCategory");
    ...
    fn reversed_crossjoin_semantic_values_not_swapped() {
        ...
        assert!(xml.contains("<Caption>Category A</Caption>"), "ProductCategory hierarchy must show category names");
    ...
    fn collapse_exclude_region_keeps_north_visible_as_all() {
        ...
        assert!(xml.contains("All"), "All caption should appear for collapsed ProductCategory");
    ...
    fn slicer_produces_malloy_total() {
        ...
        assert!(out.contains("where: product_category = 'Category A'"));
    }

    #[test]
    fn drilldown_produces_malloy_group_by_product_category() {
    ...
    fn malloy_two_dim_filtered_query() {
        ...
        assert!(out.contains("(product_category = 'Category A' or product_category = 'Category B' or product_category = 'Category D')"));
    ...
    fn generated_project_fallback_measures_return_real_data() {
        ...
        assert!(dvt_count > 0.0, "DVT measure should return > 0 order_id, got {dvt_count}");

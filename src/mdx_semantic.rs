...
fn dim_key_str(dim: &DimKey) -> String {
    match dim {
        ...
        DimKey::ProductCategory => "ProductCategory".into(),
...
fn row_dimension_from_mdx(mdx: &str) -> Option<&'static str> {
    ...
    if select_end < mdx.len() && mdx[..select_end].contains("[Region]") {
        Some("Region")
    } else if select_end < mdx.len() && mdx[..select_end].contains("[ProductCategory]") {
...
fn parse_axis_dimensions(mdx: &str) -> Vec<String> {
    ...
    // in positional order.
    let candidates: &[&str] = &["ProductCategory", "Region"];

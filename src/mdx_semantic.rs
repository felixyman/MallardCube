...
pub fn parse_mdx_filters(mdx: &str) -> Vec<String> {
    if let Some(where_payload) = where_clause_payload(mdx) {
        for dim in [PRODUCTCATEGORY_HIER, REGION_HIER] {
...
pub struct SemanticQuery {
    ...
    /// Detected dimension from MDX: "ProductCategory" or "Region".
...
fn dimension_key_from_mdx(mdx: &str) -> Option<&'static str> {
    if mdx.contains("[Region]") {
        Some("Region")
    } else if mdx.contains("[ProductCategory]") {
...
pub fn semantic_query_from_mdx(mdx: &str) -> SemanticQuery {
    ...
    } else if (mdx.contains("AddCalculatedMembers({[ProductCategory].[ProductCategory].&[") || mdx.contains("AddCalculatedMembers({[Region].[Region].&["))

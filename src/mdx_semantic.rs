...
pub const PRODUCTCATEGORY_HIER: &str = "[ProductCategory].[ProductCategory]";
pub const PRODUCTCATEGORY_ALL_U: &str = "[ProductCategory].[ProductCategory].[All]";
pub const PRODUCTCATEGORY_ALL_L: &str = "[ProductCategory].[ProductCategory].[(All)]";
pub const PRODUCTCATEGORY_LEAF_L: &str = "[ProductCategory].[ProductCategory].[ProductCategory]";
...
pub const PRODUCTCATEGORY_PROP_NAMES: &[&str] = &[
    "PARENT_UNIQUE_NAME",
...
fn dim_key_str(dim: &crate::mdx_parser::DimKey) -> String {
    match dim {
        ...
        crate::mdx_parser::DimKey::ProductCategory => "ProductCategory".into(),
...
pub struct SemanticQuery {
    ...
    /// Target hierarchy for DrilldownMember (e.g. "Region" or "ProductCategory").
...
fn row_dimension_from_mdx(mdx: &str) -> Option<&'static str> {
    ...
    if mdx[..select_end].contains("[Region]") {
        Some("Region")
    } else if mdx[..select_end].contains("[ProductCategory]") {
...
fn parse_axis_dimensions(mdx: &str) -> Vec<String> {
    ...
    for dim in &["ProductCategory", "Region"] {
...
pub fn semantic_query_from_mdx(mdx: &str) -> SemanticQuery {
    ...
    let has_visible_dim = mdx.contains("[ProductCategory]") || mdx.contains("[Region]");
    ...
    } else if (mdx.contains("AddCalculatedMembers({[ProductCategory].[ProductCategory].&[")
        ...
        || mdx.contains("AddCalculatedMembers({[Region].[Region].[Region].Members})")
    {
        ...
        mdx.contains("WHERE ([ProductCategory].[ProductCategory].[All],[Measures].[Total Sales])")
            || mdx.contains("WHERE ([Region].[Region].[All],[Measures].[Total Sales])"))
    {
...
fn parse_drilldown_member_hierarchy(mdx: &str) -> Option<String> {
    ...
    if hier == "Region" {
        Some("Region".into())
    } else if hier == "ProductCategory" {
        Some("ProductCategory".into())
    } else {
        None
    }
}

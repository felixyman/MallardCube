...
pub fn generate_rows(config: &BenchmarkDataConfig) -> Vec<FactRow> {
    ...
    let categories: Vec<String> =
        (1..=config.category_count).map(|i| format!("Category {:03}", i)).collect();
...
impl Backend {
    ...
    pub fn new_with_config(config: &BenchmarkDataConfig) -> Result<Self, rusqlite::Error> {
        ...
        let rows = generate_rows(config);
        {
            let mut stmt = conn.prepare(
                "INSERT INTO fact_table (product_category, region, sales) VALUES (?1, ?2, ?3)"
            )?;
            for row in &rows {
                stmt.execute(rusqlite::params![row.product_category, row.region, row.sales])?;
            }
        }
    ...
    pub fn sales_for_categories(&self, cats: &[String]) -> Vec<(String, f64)> {
        ...
        let sql = format!(
            "SELECT product_category, SUM(sales) FROM fact_table WHERE product_category IN ({}) GROUP BY 1 ORDER BY 1",
            placeholders.join(",")
        );
    ...
    pub fn total_for_categories(&self, cats: &[String]) -> f64 {
        ...
        let placeholders: Vec<String> = (1..=cats.len()).map(|i| format!("?{}", i)).collect();
        let sql = format!(
            "SELECT COALESCE(SUM(sales), 0) FROM fact_table WHERE product_category IN ({})",
            placeholders.join(",")
        );
        let params: Vec<&dyn rusqlite::types::ToSql> =
            cats.iter().map(|c| c as &dyn rusqlite::types::ToSql).collect();
    ...
    pub fn sales_by_category(&self) -> Vec<(String, f64)> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT product_category, SUM(sales) FROM fact_table GROUP BY 1 ORDER BY 1")
    ...
    pub fn category_names(&self) -> Vec<String> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT DISTINCT product_category FROM fact_table ORDER BY 1")
    ...
    pub fn sales_by_region(&self) -> Vec<(String, f64)> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT region, SUM(sales) FROM fact_table GROUP BY 1 ORDER BY 1")
    ...
    pub fn sales_for_regions(&self, regions: &[String]) -> Vec<(String, f64)> {
        ...
        let sql = format!(
            "SELECT region, SUM(sales) FROM fact_table WHERE region IN ({}) GROUP BY 1 ORDER BY 1",
            placeholders.join(",")
        );
        let params: Vec<&dyn rusqlite::types::ToSql> =
    ...
    pub fn total_for_regions(&self, regions: &[String]) -> f64 {
        ...
        let placeholders: Vec<String> = (1..=regions.len()).map(|i| format!("?{}", i)).collect();
        let sql = format!(
            "SELECT COALESCE(SUM(sales), 0) FROM fact_table WHERE region IN ({})",
            placeholders.join(",")
        );
        let params: Vec<&dyn rusqlite::types::ToSql> =
            regions.iter().map(|c| c as &dyn rusqlite::types::ToSql).collect();
    ...
    pub fn region_names(&self) -> Vec<String> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT DISTINCT region FROM fact_table ORDER BY 1")
    ...
    pub fn grouped_by_product_category(&self, region_filter: &[String], kat_filter: &[String]) -> Vec<(String, f64)> {
        let conn = self.conn.lock().unwrap();
        if region_filter.is_empty() && kat_filter.is_empty() {
            let mut stmt = conn
                .prepare("SELECT product_category, SUM(sales) FROM fact_table GROUP BY 1 ORDER BY 1")
                .expect("prepare grouped_by_product_category");
            let rows = stmt
                .query_map([], |row| Ok((row.get::<_, String>(0)?, row.get::<_, f64>(1)?)))
                .expect("query_map grouped_by_product_category");
        ...
        if !kat_filter.is_empty() {
            ...
            conditions.push(format!("product_category IN ({})", ph.join(",")));
            for k in kat_filter {
                params.push(Box::new(k.clone()));
            }
        }
        let sql = format!(
            "SELECT product_category, SUM(sales) FROM fact_table WHERE {} GROUP BY 1 ORDER BY 1",
            conditions.join(" AND ")
        );
        let param_refs: Vec<&dyn rusqlite::types::ToSql> = params.iter().map(|p| p.as_ref()).collect();
        let mut stmt = conn.prepare(&sql).expect("prepare grouped_by_product_category filtered");
        let rows = stmt
            .query_map(param_refs.as_slice(), |row| {
                Ok((row.get::<_, String>(0)?, row.get::<_, f64>(1)?))
            })
            .expect("query_map grouped_by_product_category filtered");
        rows.filter_map(|r| r.ok()).collect()
    }
    ...
    pub fn grouped_by_region(&self, region_filter: &[String], kat_filter: &[String]) -> Vec<(String, f64)> {
        ...
        let sql = format!(
            "SELECT region, SUM(sales) FROM fact_table WHERE {} GROUP BY 1 ORDER BY 1",
            conditions.join(" AND ")
        );
        let param_refs: Vec<&dyn rusqlite::types::ToSql> = params.iter().map(|p| p.as_ref()).collect();
    ...
    pub fn total_with_filters(&self, region_filter: &[String], kat_filter: &[String]) -> f64 {
        ...
        let sql = if conditions.is_empty() {
            "SELECT COALESCE(SUM(sales), 0) FROM fact_table".to_string()
        } else {
            format!("SELECT COALESCE(SUM(sales), 0) FROM fact_table WHERE {}", conditions.join(" AND "))
        };
        let param_refs: Vec<&dyn rusqlite::types::ToSql> = params.iter().map(|p| p.as_ref()).collect();
    ...
    pub fn grouped_pairs(&self) -> Vec<(String, String, f64)> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn
            .prepare("SELECT product_category, region, SUM(sales) FROM fact_table GROUP BY 1, 2 ORDER BY 1, 2")
    ...
    pub fn grouped_pairs_filtered(&self, region_filter: &[String], kat_filter: &[String]) -> Vec<(String, String, f64)> {
        ...
        let sql = if conditions.is_empty() {
            "SELECT product_category, region, SUM(sales) FROM fact_table GROUP BY 1, 2 ORDER BY 1, 2".to_string()
        } else {
            format!(
                "SELECT product_category, region, SUM(sales) FROM fact_table WHERE {} GROUP BY 1, 2 ORDER BY 1, 2",
                conditions.join(" AND ")
            )
        };
        let param_refs: Vec<&dyn rusqlite::types::ToSql> = params.iter().map(|p| p.as_ref()).collect();

...
impl Backend {
    ...
    pub fn grouped_by_product_category(&self, region_filter: &[String]) -> Vec<(String, f64)> {
        let conn = self.conn.lock().unwrap();
        ...
        let sql = format!(
            "SELECT product_category, SUM(sales) FROM fact_table WHERE region IN ({}) GROUP BY 1 ORDER BY 1",
            placeholders.join(",")
        );
        let params: Vec<&dyn rusqlite::types::ToSql> =
    ...
    pub fn grouped_by_region(&self, kat_filter: &[String]) -> Vec<(String, f64)> {
        ...
        let sql = format!(
            "SELECT region, SUM(sales) FROM fact_table WHERE product_category IN ({}) GROUP BY 1 ORDER BY 1",
            placeholders.join(",")
        );
        let params: Vec<&dyn rusqlite::types::ToSql> =

 ...
 */
async function withQueryResult(querySrc, tm, debug, successMessage, matchFn) {
    ...
    const isDebug = debug || (queryTestTag === null || queryTestTag === void 0 ? void 0 : queryTestTag.has('debug'));
    const fails = matchFn(runResult);
...
async function matchImpl(querySrc, tm, expectedRows, options, mode, strictRowCount, jestUtils) {
    var _a;
    return withQueryResult(querySrc, tm, (_a = options.debug) !== null && _a !== void 0 ? _a : false, mode === 'exact'
        ? 'All rows matched expected results exactly'
        : 'All rows matched expected results', ({ data, schema, dataObjects }) => {
        var _a;
        const fails = [];
        ...
        if (mode === 'partial' &&
            ...
            for (let i = 0; i < rowsToCheck; i++) {
                const cellResult = matchCell(rows[i], rootFieldInfo, expectedRows[i], `Row ${i}`, tm.dialect, mode);
                if (!cellResult.pass) {
                    // Extract field name from path like "Row 0.fieldname" or "Row 0.nested.field"
                    const pathMatch = (_a = cellResult.path) === null || _a === void 0 ? void 0 : _a.match(/^Row \d+\.(.+)$/);
                    const fieldPath = pathMatch ? pathMatch[1] : cellResult.path;
                    const issue = jestUtils.EXPECTED_COLOR(`    Expected ${fieldPath}: ${cellResult.expected}`);
                    if (!rowIssues.has(i)) {
                        rowIssues.set(i, []);
                    }
...
expect.extend({
    ...
    async toMatchPaths(querySrc, tm, paths) {
        const jestUtils = this.utils;
        return withQueryResult(querySrc, tm, false, 'All paths matched expected values', ({ dataObjects }) => {
            const fails = [];
            if (dataObjects.length === 0) {
                fails.push('Query returned no data');
                return fails;
            }
            const row = dataObjects[0];
            for (const [path, expected] of Object.entries(paths)) {
                ...
                if (!looseEqual(current, expected)) {
                    ...
                    fails.push(jestUtils.EXPECTED_COLOR(`  Expected: ${humanReadable(expected)}`));
                    fails.push(jestUtils.RECEIVED_COLOR(`  Received: ${humanReadable(current)}`));
                }
            }

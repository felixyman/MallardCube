...
FROM (... COUNT(DISTINCT f.order_id) AS base_count ...)
```
...
Use the checked-in example where the base measure `Completed Orders`
already carries `order_status = 'COMPLETED'` semantics, but the generated
cumulative fallback currently counts distinct `order_id` directly.

-- Cumulative YTD for: Antal utförda remisser (ack Kvartal) CY-1
-- Original DAX: CALCULATE( [Antal utförda remisser], FILTER( ALLSELECTED('dw_fys Kalender_Undersökningsslut'[Kvartal]), ISONORAFTER('dw_fys Kalender_Undersökningsslut'[Kvartal],    MAX('dw_fys Kalender_Undersökningsslut'[Kvartal]), DESC) ),'dw_fys Kalender_Undersökningsslut'[ÅR] = YEAR(TODAY())-1 )
